...
TEST_DIR=testes
TEST_SCRIPT=test.sh
BASE_DIR=$TEMP_DIR/prog3-tests2-$(whoami)
SUBDIR_INPUT=in
SUBDIR_OUTPUT=out
	...
	if [ -d $dir ] && [ "$dir" != "$TEST_DIR" ]; then
		...
		if [ $exitcode = 0 ]; then
			...
			for subdir in `ls $TEST_DIR`; do
				...
				# Runs the test.
				$TEST_DIR/$subdir/$TEST_SCRIPT "$subdir" "$dir" "$srcdir" "$TEMP_DIR" "$BASE_DIR" "$TEST_DIR/$subdir" "$SUBDIR_INPUT" "$SUBDIR_OUTPUT" "diff -y -W 100 --strip-trailing-cr -w -B"
...
echo "Script de teste PROG3 - Trabalho 2"
echo
